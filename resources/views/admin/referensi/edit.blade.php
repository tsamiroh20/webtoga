        ...
        <div class="d-flex justify-content-center align-items-center" style="min-height: 100vh; width: 100%; padding: 100px 0;">
            <div class="container" style="max-width: 900px; background-color: #fff; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); border-radius: 8px; padding: 20px;">
                ...
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h1 style="font-size: 1.7rem; text-align: center; width: 100%;">Edit Referensi</h1>
                </div>
                <hr />
                
                <form action="{{ route('admin/referensi/update', $referensi->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT') 
                    <!-- form update/edit data -->
                    <div class="mb-3" style="text-align: center; margin-top: 30px;">
                        <label for="referensi" class="form-label" style="font-weight: bold; color: #222021;">Referensi</label>
                        <textarea id="referensi" name="referensi" rows="3" required style="text-align: center;" class="form-control">{{ $referensi->referensi }}</textarea>
                    </div>
                    <div class="mb-3" style="text-align: center; margin-top: 30px;">
                        <label for="pustaka" class="form-label" style="font-weight: bold; color: #222021;">Pustaka</label>
                        <textarea id="pustaka" name="pustaka" rows="3" required style="text-align: center;" class="form-control">{{ $referensi->pustaka }}</textarea>
                    </div> 

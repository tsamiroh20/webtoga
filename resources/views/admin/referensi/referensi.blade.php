        ...
        <div class="d-flex justify-content-center align-items-center w-100">
            <div class="container my-5" style="max-width: 1200px; background-color: #fff; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); border-radius: 8px; padding: 20px;">
                ...
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h1 style="font-size: 1.7rem; text-align: center; width: 100%;">Data Referensi</h1>
                    <a href="{{ route('admin/referensi/create') }}" class="btn btn-primary" style="font-size: 1rem;">Tambah Referensi</a>
                </div>

                <!-- Form Pencarian -->
                <div class="d-flex justify-content-center mb-4">
                    <form class="d-flex" action="{{ route('admin/referensisearch') }}" method="GET" style="width: 70%; border-radius: 50px; overflow: hidden; background-color: #fff; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
                        <input class="form-control" type="search" placeholder="Cari data Referensi ..." aria-label="Search" name="query" style="border: none; box-shadow: none; border-radius: 50px 0 0 50px; flex-grow: 1; padding: 10px 20px;">
                    ...
                    <table class="table table-striped table-bordered table-hover text-start" style="min-width: 100%; table-layout: fixed;">
                        <thead style="background-color: #28a745; color: #fff;">
                            <tr>
                                ...
                                <th scope="col" style="text-align: center; width: 200px;">Nama Tanaman</th>
                                <th scope="col" style="text-align: center; width: 300px;">Referensi</th>
                                <th scope="col" style="text-align: center; width: 300px;">Pustaka</th>
                                <th scope="col" style="text-align: center; width: 230px;">Action</th>
                            </tr>
                        </thead>
                        ...
                        <tbody>
                            ...
                            <tr class="bg-white border-bottom">
                                ...
                                <td style="white-space: normal; overflow: hidden; text-overflow: ellipsis;">
                                    {{ $referensi->tanaman ? $referensi->tanaman->nama_latin : 'Tidak diketahui' }}
                                </td>
                                <td style="white-space: normal; overflow: hidden; text-overflow: ellipsis;">
                                    {{ $referensi->referensi }}
                                </td>
                                ...
                                <td class="w-36" style="text-align: center;">
                                    <div class="d-flex align-items-center h-14 pt-2">
                                        <a href="{{ route('admin/referensi/show', $referensi->id) }}" class="btn btn-primary me-2">Detail</a>
                                        <a href="{{ route('admin/referensi/edit', $referensi->id) }}" class="btn btn-success me-2">Edit</a>
                                        <form action="{{ route('admin/referensi/destroy', $referensi->id) }}" method="POST" onsubmit="return confirm('Delete?')" class="d-inline-block">
                                            @csrf
                                            @method('DELETE')
...
<script>
    function reloadPage() {
        window.location.href = "/admin/referensi"; // Atur ini ke URL yang menampilkan semua data
    }
</script>
